...
gSGNHT_EG <- function(dat,k, init, prior_par, alg_par, niter, updateCenter) {
  ...
  if(!is.null(prior_par$lambda_prior)) {
    phis <- matrix(NA, p, k);lambda <- matrix(NA, nrow = p, ncol = k);ts <- rgamma(p,k* prior_par$lambda_prior, 0.5);
  ...
  #### prior ####
  prior_para <- list(
    ltau = list(cvmf = init$tau, dvmf = prior_par$tau_sd,lowerB = prior_par$tau_lowerB, upperB = prior_par$tau_upperB,
                fac = 1, update = 1), # cvmf:mu; dvmf:standard dev
  ...
  res <- gSGNHT_EG_sampling(dat, k,initial_val, prior_para,
                            ...
                            alg_par$scalar_para, niter,  NULL,
...
gSGNHT_EG_sampling <- function(dat, latent_dim, variable_interest,
 ...
 # }
  ...
  fixcenter <- F; dist <- NULL; cur_elbow <- 0
  ...
  for (it in 2:(niter + 1)) {
   ...
   # if ((it < 0.3 * niter)) {
    ...
    temp <- updateSample_gSGNHT(temp$par,par_type, para_dims, para_dim_updated,
                                ...
                                prior_para, accurate_grad,mode_grad,
                                minibatchSize, upweight_idx, temp$SS, it - 1, temp$covPara,Lsteps = 10) # (CHANGE) returning scaled para
  ...
  #  if (initialization_period) {temp$par$linvSig <- variable_interest$linvSig; temp$par$ltau <- variable_interest$ltau}
    ...
    if (!is.null(mis_res) && it > 100) {
      dat[mis_res$misPos] <- predictY(temp$par, prior_para$ltau, dat[mis_res$na_yes,,drop = F],mis_res$misIdx, mis_res$misPos_relative2Mis)
...
updateSample_gSGNHT <- function(all_theta, par_type, para_dims, para_dim_updated,
                                ...
                                mode_grad,minibatchSize, initialization_period, SS = NULL,
                                iteration = NULL,
                                covPara = NULL, Lsteps = 10) {
  N <- nrow(dat)
  if (!is.null(covPara)) {
    covCtrl <- T; covCtrlPar <- list(N = N, minibatchsize = minibatchSize, covPara = covPara)
  }
  else {
    covCtrl <- F; covCtrlPar <- NULL
  }
  ltau_ <- ifelse(prior_para$ltau$update, 'non','ltau')
  ...
  for (l in 1:Lsteps) {
   ...
   v <-  updateV_stepB_ctrlCov(epsilon / 2, res$xi, res$aux, covCtrlPar)
      ...
      v_list <- updateV_stepO_fast(dat, para_dims, k, par_type, all_theta,
                                   ...
                                   accurate_grad, mode_grad,
                                   minibatchSize, initialization_period, covCtrl,print_status)
...
updateV_stepO_fast <- function(dat, para_dim, k,  par_notin_euclidean,
                               all_theta, v, epsilon, Apara,
                               prior_para, accurate_grad,
                               mode_grad,
                               minibatchSize, initialization_period, controlVar = F, print_status = F) {
  ...
  if (controlVar) {
    ...
    dUtilde <- calcSGgrad(all_theta, dat, k, prior_para, accurate_grad = accurate_grad,
                          minibatchSize, initialization_period, prior_exclude = F, print_status = print_status)
   # dUtilde <- calcSGgrad_integrateOutCenter(all_theta, dat, k, prior_para, accurate_grad = accurate_grad,
                         # minibatchSize, prior_exclude = F)
    #### orignal function
    #dUtilde <- calcSGgrad(all_theta, dat, k, prior_para, accurate_grad = accurate_grad,
  ...
  term1 <- if(controlVar) lapply(dUtilde$grad, '*', -epsilon) else lapply(dUtilde, '*', -epsilon)
  # term1 <- -epsilon *dUtilde # make it so that it is a list
 # term2 <- if(controlVar) randomDrift_fast(Apara, epsilon, para_dim,
                                         #  Bmat = dUtilde$gradCov) else randomDrift_fast(Apara, epsilon, para_dim,
                                                                  #   Bmat = 0)
...
randomDrift_fast <- function(Apara, epsilon, para_dim, Bmat = 0) {
  ...
  for (i in seq_along(res)) {
    if (length(para_dim[[i]]) == 1) {
      res[[i]] <-  rnorm(para_dim[[i]], mean = mean_, sd = std_)
    }
    else {
      res[[i]] <- rnorm(prod(para_dim[[i]]), mean = mean_, sd = std_)
...
updateCovPara <- function(current_cov_para,
                          current_iteration,
                          gradCov) {
  # computes I_t
#  kap <- 0.1
 # kap <- 0.99
    kap <- 1 / current_iteration
#  res <- (1 - kap) * current_cov_para + kap * gradCov
  name_ <- intersect(names(current_cov_para),names(gradCov))
  res <- mapply(function(x,y) {(1 - kap) * x + kap * y}, x = gradCov[name_],
         y = current_cov_para[name_])
  # cutoff <- 1e6
  # res[res > cutoff] <- cutoff
  # res[res < -cutoff] <- -cutoff
  # try and constrain this guy - when does this guy create problem?
  return(res)
}
...
tau2transformedTau<- function(tau, tau_bounds) {
  ...
  temp <- car::logit((tau - tau_bounds$lowerB) / (tau_bounds$upperB - tau_bounds$lowerB))

  return(temp * tau_bounds$fac)
}
#saveUpdate <- function(sampleList, rowIdx,cur_sample) {
 # for(i in seq_along(sampleList)) {
  #  var_ <- names(samplesList)[i]
  #  if (var_)
  #  sampleList[[var_]][rowIdx, ]
  #}
#}
...
logDensityTau_prior <- function(tau_prop, tau_prior) {
  ...
  res <- dgamma(tau_prop, tau_prior$cvmf, tau_prior$dvmf, log = T)
  return(res)
}
logDensitylinvSig <- function(linvSig, asig) {
  sum(dnorm(exp(-linvSig), mean = 0, sd = asig, log = T) - linvSig)
}
